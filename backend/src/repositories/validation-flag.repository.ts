...
export class ValidationFlagRepository {
  ...
  async create(
    data: Partial<ValidationFlagEntity>,
  ): Promise<ValidationFlagEntity> {
  ...
  async findByEntity(
    ...
    return this.repository.find({
      where: { entityType, entityId } as FindOptionsWhere<ValidationFlagEntity>,
    });
  }
  ...
  async findBySeverity(
    ...
    return this.repository.find({
      where: { severity } as FindOptionsWhere<ValidationFlagEntity>,
    });
  }

  async findByFlagType(flagType: string): Promise<ValidationFlagEntity[]> {
    return this.repository.find({
      where: { flagType } as FindOptionsWhere<ValidationFlagEntity>,
    });
  }
  ...
  async findAll(options?: {
    skip?: number;
    take?: number;
  }): Promise<[ValidationFlagEntity[], number]> {
