...
export class CsvRowValidator {
  validate(row: RawCsvRow): CsvRowValidationFailure[] {
    ...
    if (row.values.every((value) => value.trim().length === 0) && row.extraValues.length === 0) {
      failures.push(this.createFailure(row, 'Row is empty.'));
    }

    const missingHeaders = row.headers.filter((header) => !(header in row.raw));
    if (missingHeaders.length > 0) {
      failures.push(this.createFailure(row, `Missing columns: ${missingHeaders.join(', ')}.`));
    }

    if (row.extraValues.length > 0) {
      failures.push(this.createFailure(row, `Extra columns detected: ${row.extraValues.length}.`));
    }

    return failures;
  }

  private createFailure(row: RawCsvRow, reason: string): CsvRowValidationFailure {
    return {
